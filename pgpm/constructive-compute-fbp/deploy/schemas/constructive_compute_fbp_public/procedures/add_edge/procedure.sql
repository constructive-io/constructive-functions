-- Deploy: schemas/constructive_compute_fbp_public/procedures/add_edge/procedure
-- made with <3 @ constructive.io

-- requires: schemas/constructive_compute_fbp_public/schema


CREATE FUNCTION "constructive_compute_fbp_public".add_edge(
  IN database_id uuid,
  IN root_hash uuid,
  IN src_node text,
  IN src_port text,
...
BEGIN
  FOR v_row IN SELECT *
  FROM "constructive_compute_fbp_public".platform_function_graph_get_all(add_edge.database_id, add_edge.root_hash)
  WHERE
  ...
  RETURN "constructive_compute_fbp_public".platform_function_graph_insert_node_at_path(add_edge.database_id, add_edge.root_hash, v_scope_path, v_edge_data, '{}'::uuid[], '{}'::text[]);
END;
