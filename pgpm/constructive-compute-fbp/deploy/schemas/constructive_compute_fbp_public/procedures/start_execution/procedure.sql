-- Deploy: schemas/constructive_compute_fbp_public/procedures/start_execution/procedure
-- made with <3 @ constructive.io

-- requires: schemas/constructive_compute_fbp_public/schema


CREATE FUNCTION "constructive_compute_fbp_public".start_execution(
  IN graph_id uuid,
...
BEGIN
  ...
  WHERE
    ...
    END IF;
  END LOOP;
  INSERT INTO "constructive_compute_fbp_private".function_graph_executions (
    graph_id,
    database_id,
    output_node,
    output_port,
  ...
  PERFORM "constructive_compute_fbp_private".tick_execution(v_exec_id);
  RETURN v_exec_id;
END;
