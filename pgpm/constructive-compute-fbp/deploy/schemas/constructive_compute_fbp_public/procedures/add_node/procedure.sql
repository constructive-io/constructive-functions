-- Deploy: schemas/constructive_compute_fbp_public/procedures/add_node/procedure
-- made with <3 @ constructive.io

-- requires: schemas/constructive_compute_fbp_public/schema


CREATE FUNCTION "constructive_compute_fbp_public".add_node(
  IN database_id uuid,
  IN root_hash uuid,
  IN node_name text,
  IN node_type text,
...
BEGIN
  ...
  END IF;
  RETURN "constructive_compute_fbp_public".platform_function_graph_insert_node_at_path(add_node.database_id, add_node.root_hash, v_path, v_node_data, '{}'::uuid[], '{}'::text[]);
END;
