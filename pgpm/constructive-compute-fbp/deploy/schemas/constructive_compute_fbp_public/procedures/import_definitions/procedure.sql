-- Deploy: schemas/constructive_compute_fbp_public/procedures/import_definitions/procedure
-- made with <3 @ constructive.io

-- requires: schemas/constructive_compute_fbp_public/schema


CREATE FUNCTION "constructive_compute_fbp_public".import_definitions(
  IN graph_id uuid,
...
BEGIN
  ...
  FOR v_i IN 1..cardinality(import_definitions.contexts) LOOP
    ...
    v_root_hash := "constructive_compute_fbp_private".copy_subtree(import_definitions.source_scope_id, import_definitions.source_commit_id, ARRAY[v_ctx, 'definitions'], v_graph.database_id, v_root_hash, ARRAY[v_ctx, 'definitions']);
  END LOOP;
  PERFORM "constructive_compute_fbp_public".save_graph(import_definitions.graph_id, v_root_hash, 'import definitions');
  RETURN;
END;
