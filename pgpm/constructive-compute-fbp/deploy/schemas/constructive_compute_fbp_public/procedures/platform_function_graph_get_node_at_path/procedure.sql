-- Deploy: schemas/constructive_compute_fbp_public/procedures/platform_function_graph_get_node_at_path/procedure
-- made with <3 @ constructive.io

-- requires: schemas/constructive_compute_fbp_public/schema


CREATE FUNCTION "constructive_compute_fbp_public".platform_function_graph_get_node_at_path(
  IN s_id uuid,
  IN id uuid,
  IN path text[] DEFAULT ARRAY[]::text[]
) RETURNS "constructive_compute_fbp_public".platform_function_graph_object AS $_PGFN_$
DECLARE
  _path text[] := path;
  _obj "constructive_compute_fbp_public".platform_function_graph_object;
  i int;
  pos int;
  curpath text;
  _node_id uuid;
BEGIN
  ...
  WHERE
    o.id = platform_function_graph_get_node_at_path.id AND o.database_id = s_id INTO _obj;
