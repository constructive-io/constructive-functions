-- Deploy: schemas/constructive_compute_fbp_private/procedures/serialize_graph/procedure
-- made with <3 @ constructive.io

-- requires: schemas/constructive_compute_fbp_private/schema


CREATE FUNCTION "constructive_compute_fbp_private".serialize_graph(
  IN graph_id uuid
...
BEGIN
  ...
  WHERE
    id = serialize_graph.graph_id INTO v_graph;
  IF NOT (FOUND) THEN
  ...
  FOR v_row IN SELECT
    path,
    data
  FROM "constructive_compute_fbp_public".platform_function_graph_get_all(v_graph.database_id, v_tree_id) LOOP
