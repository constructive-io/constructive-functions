...
BEGIN
  ...
  WHERE
    s.database_id = platform_create_function_graph.database_id AND s.name = platform_create_function_graph.name INTO v_store_id;
...
ALTER TABLE constructive_compute_private.platform_function_graph_executions 
  ADD COLUMN execution_store_id uuid;

COMMENT ON COLUMN constructive_compute_private.platform_function_graph_executions.execution_store_id IS 'FK to execution_tree_store — links execution to its Merkle tree for time-travel debugging';
...
COMMENT ON TABLE constructive_compute_private.platform_execution_tree_object IS 'Content-addressed execution tree nodes — partitioned mirror of merkle object table';
...
COMMENT ON COLUMN constructive_compute_private.platform_execution_tree_object.id IS 'Content-addressed UUID v5 (hash of data + children)';
...
COMMENT ON COLUMN constructive_compute_private.platform_execution_tree_object.database_id IS 'Scope for multi-tenant isolation';

ALTER TABLE constructive_compute_private.platform_execution_tree_object 
  ADD COLUMN kids uuid[];

COMMENT ON COLUMN constructive_compute_private.platform_execution_tree_object.kids IS 'Child object IDs (ordered)';

ALTER TABLE constructive_compute_private.platform_execution_tree_object 
  ADD COLUMN ktree text[];

COMMENT ON COLUMN constructive_compute_private.platform_execution_tree_object.ktree IS 'Child path segment names (parallel to kids)';

ALTER TABLE constructive_compute_private.platform_execution_tree_object 
  ADD COLUMN data jsonb;

COMMENT ON COLUMN constructive_compute_private.platform_execution_tree_object.data IS 'Node data payload';

COMMENT ON COLUMN constructive_compute_private.platform_execution_tree_object.created_at IS 'Timestamp of object creation (partition key)';

ALTER TABLE constructive_compute_private.platform_execution_tree_object 
  ADD CONSTRAINT platform_execution_tree_objects_pkey PRIMARY KEY (created_at, id, database_id);

CREATE TABLE constructive_compute_private.platform_execution_tree_store (
  created_at timestamptz NOT NULL DEFAULT now()
) PARTITION BY RANGE (created_at);
...
COMMENT ON TABLE constructive_compute_private.platform_execution_tree_store IS 'One store per execution — tracks current root hash';
...
COMMENT ON COLUMN constructive_compute_private.platform_execution_tree_store.id IS 'Unique store identifier';
...
COMMENT ON COLUMN constructive_compute_private.platform_execution_tree_store.database_id IS 'Scope for multi-tenant isolation';
...
COMMENT ON COLUMN constructive_compute_private.platform_execution_tree_store.name IS 'Store name (typically the execution ID)';
...
COMMENT ON COLUMN constructive_compute_private.platform_execution_tree_store.created_at IS 'Timestamp of store creation (partition key)';

ALTER TABLE constructive_compute_private.platform_execution_tree_store 
  ADD CONSTRAINT platform_execution_tree_stores_pkey PRIMARY KEY (created_at, id);

CREATE TABLE constructive_compute_private.platform_execution_tree_commit (
  created_at timestamptz NOT NULL DEFAULT now()
) PARTITION BY RANGE (created_at);
...
COMMENT ON TABLE constructive_compute_private.platform_execution_tree_commit IS 'One commit per tick — time-travel snapshots of execution tree state';
...
COMMENT ON COLUMN constructive_compute_private.platform_execution_tree_commit.id IS 'Unique commit identifier';
...
COMMENT ON COLUMN constructive_compute_private.platform_execution_tree_commit.database_id IS 'Scope for multi-tenant isolation';
...
COMMENT ON COLUMN constructive_compute_private.platform_execution_tree_commit.store_id IS 'FK to execution_tree_store';
...
COMMENT ON COLUMN constructive_compute_private.platform_execution_tree_commit.tree_id IS 'Root object ID — tree state at this commit';

ALTER TABLE constructive_compute_private.platform_execution_tree_commit 
  ADD COLUMN parent_ids uuid[];

COMMENT ON COLUMN constructive_compute_private.platform_execution_tree_commit.parent_ids IS 'Parent commit IDs (links to previous tick commit)';

ALTER TABLE constructive_compute_private.platform_execution_tree_commit 
  ADD COLUMN message text;

COMMENT ON COLUMN constructive_compute_private.platform_execution_tree_commit.message IS 'Commit message (e.g. "tick 3: completed add_node, double_node")';
...
COMMENT ON COLUMN constructive_compute_private.platform_execution_tree_commit.date IS 'Commit timestamp';

COMMENT ON COLUMN constructive_compute_private.platform_execution_tree_commit.created_at IS 'Row creation timestamp (partition key)';

ALTER TABLE constructive_compute_private.platform_execution_tree_commit 
  ADD CONSTRAINT platform_execution_tree_commits_pkey PRIMARY KEY (created_at, id);

CREATE INDEX platform_execution_tree_commits_store_id_created_at_idx ON constructive_compute_private.platform_execution_tree_commit (store_id, created_at);

CREATE TABLE constructive_compute_private.platform_execution_tree_ref (
  created_at timestamptz NOT NULL DEFAULT now()
) PARTITION BY RANGE (created_at);
...
COMMENT ON TABLE constructive_compute_private.platform_execution_tree_ref IS 'Branch head pointers — updated after each tick commit';
...
COMMENT ON COLUMN constructive_compute_private.platform_execution_tree_ref.id IS 'Unique ref identifier';
...
COMMENT ON COLUMN constructive_compute_private.platform_execution_tree_ref.database_id IS 'Scope for multi-tenant isolation';
...
COMMENT ON COLUMN constructive_compute_private.platform_execution_tree_ref.store_id IS 'FK to execution_tree_store';
...
COMMENT ON COLUMN constructive_compute_private.platform_execution_tree_ref.name IS 'Ref name (default: main)';

COMMENT ON COLUMN constructive_compute_private.platform_execution_tree_ref.created_at IS 'Row creation timestamp (partition key)';

ALTER TABLE constructive_compute_private.platform_execution_tree_ref 
  ADD CONSTRAINT platform_execution_tree_refs_pkey PRIMARY KEY (created_at, id);

CREATE INDEX platform_execution_tree_refs_store_id_name_idx ON constructive_compute_private.platform_execution_tree_ref (store_id, name);
