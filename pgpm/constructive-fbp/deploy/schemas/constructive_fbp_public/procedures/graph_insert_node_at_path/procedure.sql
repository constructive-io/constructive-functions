-- Deploy: schemas/constructive_fbp_public/procedures/graph_insert_node_at_path/procedure
-- made with <3 @ constructive.io

-- requires: schemas/constructive_fbp_public/schema


CREATE FUNCTION "constructive_fbp_public".graph_insert_node_at_path(
  IN s_id uuid,
  IN root uuid,
  IN path text[],
  IN data jsonb,
...
DECLARE
  _newnode_id uuid;
  _newparent_id uuid;
  _parent "constructive_fbp_public".graph_object;
  _orig_name text;
  _repl uuid;
  ...
  _pos int;
  _cur_id uuid;
  _cur_obj "constructive_fbp_public".graph_object;
...
BEGIN
  ...
  WHERE
    o.id = graph_insert_node_at_path.root AND o.database_id = s_id INTO _cur_obj;
  ...
  VALUES
    (s_id, graph_insert_node_at_path.data, graph_insert_node_at_path.kids, graph_insert_node_at_path.ktree)
