-- Deploy: schemas/constructive_fbp_public/procedures/add_edge_and_save/procedure
-- made with <3 @ constructive.io

-- requires: schemas/constructive_fbp_public/schema


CREATE FUNCTION "constructive_fbp_public".add_edge_and_save(
  IN graph_id uuid,
  IN src_node text,
  IN src_port text,
  IN dst_node text,
...
BEGIN
  ...
  v_root_hash := "constructive_fbp_public".add_edge(v_graph.database_id, v_root_hash, add_edge_and_save.src_node, add_edge_and_save.src_port, add_edge_and_save.dst_node, add_edge_and_save.dst_port, v_graph.context, v_graph.name);
  PERFORM "constructive_fbp_public".save_graph(add_edge_and_save.graph_id, v_root_hash, coalesce(add_edge_and_save.message, 'add edge' || add_edge_and_save.src_node));
