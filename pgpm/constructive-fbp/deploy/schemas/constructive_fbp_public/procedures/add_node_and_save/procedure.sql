-- Deploy: schemas/constructive_fbp_public/procedures/add_node_and_save/procedure
-- made with <3 @ constructive.io

-- requires: schemas/constructive_fbp_public/schema


CREATE FUNCTION "constructive_fbp_public".add_node_and_save(
  IN graph_id uuid,
  IN node_name text,
  IN node_type text,
...
BEGIN
  ...
  v_root_hash := "constructive_fbp_public".add_node(v_graph.database_id, v_root_hash, add_node_and_save.node_name, add_node_and_save.node_type, v_graph.context, v_graph.name, add_node_and_save.props, add_node_and_save.meta);
  PERFORM "constructive_fbp_public".save_graph(add_node_and_save.graph_id, v_root_hash, coalesce(add_node_and_save.message, 'add node: ' || add_node_and_save.node_name));
