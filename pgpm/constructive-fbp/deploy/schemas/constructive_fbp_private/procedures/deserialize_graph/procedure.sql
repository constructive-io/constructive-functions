-- Deploy: schemas/constructive_fbp_private/procedures/deserialize_graph/procedure
-- made with <3 @ constructive.io

-- requires: schemas/constructive_fbp_private/schema


CREATE FUNCTION "constructive_fbp_private".deserialize_graph(
  IN database_id uuid,
  IN name text,
  IN snapshot jsonb
...
BEGIN
  ...
  PERFORM "constructive_fbp_public".graph_init_empty_repo(deserialize_graph.database_id, v_store_id);
  FOR v_row IN SELECT *
  FROM jsonb_each(deserialize_graph.tree_data->'tree') LOOP
    v_entry := v_row.value;
    v_root_hash := "constructive_fbp_public".graph_insert_node_at_path(deserialize_graph.database_id, v_root_hash, (v_entry->'path')::text[], v_entry->'data', '{}'::uuid[], '{}'::text[]);
  END LOOP;
  ...
  UPDATE "constructive_fbp_public".graph_ref AS r SET
  commit_id = (SELECT id
  FROM "constructive_fbp_public".graph_commit
  WHERE
