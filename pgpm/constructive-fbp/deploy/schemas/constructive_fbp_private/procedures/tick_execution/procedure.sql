-- Deploy: schemas/constructive_fbp_private/procedures/tick_execution/procedure
-- made with <3 @ constructive.io

-- requires: schemas/constructive_fbp_private/schema


CREATE FUNCTION "constructive_fbp_private".tick_execution(
  IN execution_id uuid
) RETURNS integer AS $_PGFN_$
DECLARE
  v_exec "constructive_fbp_private".function_graph_executions;
...
BEGIN
  SELECT *
  FROM "constructive_fbp_private".function_graph_executions
  WHERE
    id = tick_execution.execution_id INTO v_exec;
  IF NOT (FOUND) THEN
  ...
  FOR v_i IN 0..jsonb_array_length(v_nodes) - 1 LOOP
    ...
    END IF;
    SELECT data
    FROM "constructive_fbp_public".graph_get_node_at_path(v_graph.database_id, v_tree_id, ARRAY[v_graph.context, 'definitions', v_node_type]) AS def INTO v_def_data;
    IF v_def_data IS NULL THEN
      SELECT data
      FROM "constructive_fbp_public".graph_get_node_at_path(v_graph.database_id, v_tree_id, ARRAY[v_graph.context, 'definitions', v_node_name]) AS def INTO v_def_data;
    END IF;
    IF v_def_data IS NOT NULL AND v_def_data ? 'graph' THEN
      SELECT "constructive_fbp_public".import_graph_json(v_graph.database_id, ('def_' || tick_execution.execution_id) || ('_' || v_node_name), v_def_data->'graph') INTO v_sub_graph_id;
      PERFORM "constructive_fbp_public".start_execution(graph_id:=v_sub_graph_id, input_payload:=v_inputs, parent_execution_id:=tick_execution.execution_id, parent_node_name:=v_node_name);
    ...
    FROM app_jobs.jobs
    WHERE
      task_identifier LIKE ( 'fbp:eval:%' ) AND (payload::jsonb->>'execution_id')::uuid = tick_execution.execution_id INTO v_pending_jobs;
  ...
  IF v_jobs_enqueued = 0 AND v_exec.node_outputs ? v_exec.output_node THEN
    ...
    IF v_output_data IS NOT NULL THEN
      ...
      IF v_exec.parent_execution_id IS NOT NULL THEN
        PERFORM "constructive_fbp_private".complete_node(v_exec.parent_execution_id, v_exec.parent_node_name, v_output_data);
      END IF;
    END IF;
  END IF;
  RETURN v_jobs_enqueued;
