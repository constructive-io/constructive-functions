-- Deploy: schemas/constructive_fbp_private/procedures/copy_subtree/procedure
-- made with <3 @ constructive.io

-- requires: schemas/constructive_fbp_private/schema


CREATE FUNCTION "constructive_fbp_private".copy_subtree(
...
BEGIN
  ...
  FOR v_row IN SELECT
    path,
    data
  FROM "constructive_fbp_public".graph_get_all(copy_subtree.source_scope_id, v_source_tree_id)
  WHERE
    ...
    v_root_hash := "constructive_fbp_public".graph_insert_node_at_path(copy_subtree.target_scope_id, v_root_hash, v_new_path, v_row.data, '{}'::uuid[], '{}'::text[]);
  END LOOP;
  RETURN v_root_hash;
END;
