...
# Defaults / flags (can be overridden via env or CLI)
KNATIVE_ONLY=false
# Default Knative domain:
# - For internal-only routing, use the cluster-local DNS suffix so
#   KSvc URLs look like: http://service.namespace.svc.cluster.local
# - Can be overridden via KNATIVE_DOMAIN for external domains.
KNATIVE_DOMAIN="${KNATIVE_DOMAIN:-svc.cluster.local}"
...
install_knative_serving() {
    ...
    # Extra safety: explicitly wait for Route CRD to be fully established
    kubectl wait --for=condition=Established crd/routes.serving.knative.dev --timeout=120s || \
        log $YELLOW "Timed out waiting for routes.serving.knative.dev CRD; please check manually."
    ...
    # Default is svc.cluster.local (internal-only, cluster DNS),
    # but can be overridden via KNATIVE_DOMAIN for external domains.
    log $BLUE "Configuring Knative domain..."
    local domain_patch
    domain_patch=$(cat <<EOF
{"data":{"${KNATIVE_DOMAIN}":"","127.0.0.1.nip.io":null}}
EOF
)
...
verify_installations() {
    ...
    # Check core Knative CRDs (especially Route)
    if kubectl get crd routes.serving.knative.dev &> /dev/null; then
        log $GREEN "✓ Knative Route CRD (routes.serving.knative.dev) is present"
    else
        log $RED "✗ Knative Route CRD (routes.serving.knative.dev) is missing"
    fi

    # Check CloudNativePG
    if kubectl get pods -n cnpg-system 2>/dev/null | grep -q Running; then
    ...
    if kubectl get pods -n knative-serving 2>/dev/null | grep -q Running; then
    ...
    # Check Kourier
    if kubectl get pods -n kourier-system 2>/dev/null | grep -q Running; then
...
main() {
    ...
    # Ask if user wants to install Knative CLI (interactive shells only)
    if [ -t 0 ]; then
        read -p "Do you want to install Knative CLI (kn)? [y/N]: " -n 1 -r || true
        echo
        if [[ ${REPLY:-} =~ ^[Yy]$ ]]; then
            install_kn_cli
        fi
    else
        log $BLUE "Skipping Knative CLI (kn) prompt in non-interactive environment."
    fi
    ...
    if [[ "$KNATIVE_ONLY" == "true" ]]; then
        ...
        log $BLUE "You can now deploy PostgreSQL using the cnpg-cluster.yaml manifest (for example: 'cd interweb/k8s && make install')."
    else
        log $GREEN "=== Operator installation completed successfully! ==="
        log $BLUE "You can now deploy the PostgreSQL cluster using the cnpg-cluster.yaml manifest (e.g. 'cd interweb/k8s && make install')."
    fi
}
