  ...
  return (
    <div className="flex flex-col h-full bg-zinc-950 border-l border-zinc-800">
      <div className="p-3 border-b border-zinc-800">
        ...
        <div className="font-semibold text-white flex items-center gap-1">
          {nodeIsBoundary && selectedNode && (
            <span className="text-zinc-500">{selectedNode.type}:</span>
          )}
          {nodeIsBoundary ? (
            isEditingName ? (
              <input
                ...
                className="bg-zinc-900 border border-blue-500 rounded px-1 py-0.5 text-white text-sm focus:outline-none"
                style={{ width: `${Math.max(editedName.length, 5) * 8 + 16}px` }}
              />
            ) : (
              <span 
                onClick={() => setIsEditingName(true)}
                className="cursor-pointer hover:bg-zinc-800 px-1 py-0.5 rounded"
        ...
        <div className="text-xs text-zinc-400 mt-1 font-mono">{node.type}</div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-3">
        ...
        {propDefs.length === 0 ? (
          <div className="text-zinc-500 text-sm">No properties defined</div>
        ) : (
        ...
        {node.type === 'graphOutput' && (
          <div className="mt-6 pt-4 border-t border-zinc-800">
            <div className="flex items-center justify-between mb-3">
              <span className="text-xs text-zinc-500 uppercase tracking-wider">Evaluated Result</span>
              <button
            ...
            {isEvaluating ? (
              <div className="text-zinc-500 text-sm">Evaluating...</div>
            ) : evaluationResult !== undefined ? (
              <div className="bg-zinc-900 rounded p-3 overflow-auto max-h-64">
              ...
              <div className="text-zinc-500 text-sm">Click refresh to evaluate</div>
            )}
          </div>
        )}
      </div>
        ...
        return (
          <button
            ...
            className={clsx(
              ...
              displayValue ? 'bg-blue-500' : 'bg-zinc-700'
            )}
          >
            <span
              className={clsx(
      ...
      case 'number':
        return (
          <input
            ...
            className={clsx(
              'w-full px-2 py-1.5 rounded bg-zinc-900 border border-zinc-700',
      ...
      case 'select':
        ...
        return (
          <select
            ...
            className="w-full px-2 py-1.5 rounded bg-zinc-900 border border-zinc-700 text-white text-sm focus:outline-none focus:border-blue-500"
          >
  ...
  return (
    <div>
      <label className="flex items-center justify-between mb-1.5">
        <span className="text-sm text-zinc-300">{definition.name}</span>
        {isChannelRef && (
      ...
      {definition.description && (
        <p className="text-xs text-zinc-500 mt-1">{definition.description}</p>
      )}
    </div>
  );
}
