...
function HotkeyItem({ keys, description }: HotkeyItemProps) {
  return (
    <div className="flex items-center gap-2 text-xs">
      <kbd className="px-1.5 py-0.5 bg-zinc-800 rounded text-zinc-300 font-mono text-[10px]">
        {keys}
      </kbd>
...
function NodePaletteItem({ definition, onDragStart }: NodePaletteItemProps) {
  ...
  return (
    <div
      ...
      className="px-2 py-1.5 bg-zinc-800 hover:bg-zinc-700 rounded cursor-grab text-xs text-zinc-300 transition-colors"
  ...
  return (
    <div className="absolute top-0 left-0 right-0 z-10 pointer-events-none">
      <div className="flex justify-between p-3 gap-3">
        <div className="pointer-events-auto bg-zinc-950/95 backdrop-blur rounded-lg border border-zinc-800 shadow-xl">
          <button
            onClick={() => setShowPalette(!showPalette)}
            className="w-full px-3 py-2 text-xs font-medium text-zinc-300 hover:bg-zinc-900 rounded-t-lg flex items-center justify-between"
          >
            <span>Node Palette</span>
            <span className="text-zinc-500">{showPalette ? '−' : '+'}</span>
          </button>
          
          {showPalette && (
            <div className="p-2 border-t border-zinc-800 max-h-80 overflow-y-auto">
                  ...
                  <div className="flex flex-col gap-1">
                    {defs.map(def => (
                      <div
                        ...
                        className="px-2 py-1.5 bg-zinc-800 hover:bg-zinc-700 rounded cursor-pointer text-xs text-zinc-300 transition-colors"
              ...
              {definitions.length === 0 && (
                <div className="text-xs text-zinc-500 px-1">No node definitions</div>
              )}
            </div>
          )}
        </div>
        ...
        <div className="pointer-events-auto bg-zinc-950/95 backdrop-blur rounded-lg border border-zinc-800 shadow-xl">
          ...
          >
            ...
            <span className="text-zinc-500">{showHotkeys ? '−' : '+'}</span>
          </button>
          
          {showHotkeys && (
            <div className="p-3 border-t border-zinc-800 space-y-2">
              <div className="text-[10px] uppercase tracking-wider text-zinc-500 mb-2">Selection</div>
              ...
              <div className="text-[10px] uppercase tracking-wider text-zinc-500 mt-3 mb-2">Editing</div>
              <HotkeyItem keys="Delete" description="Delete selected" />
              <HotkeyItem keys="⌘/Ctrl+D" description="Duplicate" />
              
              <div className="text-[10px] uppercase tracking-wider text-zinc-500 mt-3 mb-2">Navigation</div>
              ...
              <div className="text-[10px] uppercase tracking-wider text-zinc-500 mt-3 mb-2">Connections</div>
