...
function chunkByParagraph(text: string, chunkSize: number, chunkOverlap: number): string[] {
  ...
  for (const paragraph of paragraphs) {
    ...
    overlapBuffer.push(trimmedParagraph);
...
const handler: FunctionHandler<GenerateChunksParams> = async (params, context) => {
  ...
  const isDryRun = env.RAG_EMBEDDING_DRY_RUN === 'true';
  ...
  // Find tables by query field name, table name, or inflection
  ...
  const parentTable = tables.find(t =>
    ...
    t.name?.toLowerCase() === parentQueryName ||
  ...
  const chunksTable = tables.find(t =>
    ...
    t.name?.toLowerCase() === chunksQueryName ||
    t.name?.toLowerCase() === chunks_table.toLowerCase()
  );
  ...
  for (let i = 0; i < textChunks.length; i++) {
    ...
    if (isDryRun) {
      ...
      log.info('[generate-chunks] DRY_RUN: mock embedding', { chunk_index: i });
    } else {
      try {
