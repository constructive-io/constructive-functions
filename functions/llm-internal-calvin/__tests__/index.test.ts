...
describe('LLM Internal Calvin Function (Integration)', () => {
    ...
    beforeAll(async () => {
        ...
        k8s = new KubernetesClient({ restEndpoint: 'http://127.0.0.1:8006' } as any);
    });
    ...
    it('should orchestrate the llm-internal-calvin job', async () => {
                ...
                template: {
                    ...
                    spec: {
                        ...
                        containers: [{
                            ...
                            command: ["npx", "ts-node", "functions/llm-internal-calvin/src/index.ts"],
                            env: [{ name: "CALVIN_API_KEY", value: "calvin-mock-key" }, { name: "PORT", value: "8080" }]
                        }]
                    }
                }
            }
        ...
        if (!success) throw new Error(`LLM Calvin Service Failed: ${logsResponse}`);
